...
const demoTemplates = [
  ...
  `,
    data: {
      ...
      label: 'NPM DEMO',
      title: 'npm 包模式宿主测试',
      body: '当前页面通过 miniprogram_npm/mp-components-to-canvas-or-image 引用分享卡片组件，并用 npm 包 API 做渲染预检。',
      author: 'mp-components-to-canvas-or-image',
      date: '2026.07',
    },
  },
  {
  ...
  `,
    data: {
      ...
      left: 'mp-components-to-canvas-or-image',
      right: 'Canvas DSL',
    },
  },
  {
...
Page({
  ...
  onShareCardReady() {
    this.setData({ componentReady: true, status: 'npm 包组件已挂载' });
  },
  ...
  async renderByPackageApi() {
    const template = this.getSelectedTemplate();
    this.setData({ apiRendering: true, status: 'npm 包 API 编译中' });
    try {
      ...
      this.setData({
        status: `npm 包 API 可用：生成 ${result.commands.length} 条绘制指令`,
      });
    } catch (error) {
      this.setData({ status: error && error.message ? error.message : 'npm 包 API 调用失败' });
